...
const round = ref(0)
const phase = ref('ready') // ready | showing | input | over
...
useGameLoop((dt) => {
  ...
  for (let i = 0; i < GRID * GRID; i++) {
    ...
    ctx.fillStyle = flash > 0.05 ? color + Math.floor(40 + flash * 180).toString(16).padStart(2, '0') : 'rgba(140, 180, 255, 0.06)'
    ctx.fill()
    ctx.lineWidth = 2
    ctx.strokeStyle = flash > 0.05 ? color : 'rgba(140, 180, 255, 0.18)'
    ctx.shadowColor = color
    ctx.shadowBlur = flash * 34
    ctx.stroke()
    ctx.shadowBlur = 0
  }
